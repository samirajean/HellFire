...
GLuint loadTexture(char* location, bool isIn);
...
bool CMD2Model::LoadModel(  char *filename )
{
...
bool CMD2Model::LoadSkin(  char *filename )
{
	...
	m_texid = loadTexture( filename, true );

	return (m_texid != loadTexture( "default", true ));
}
...
void CMD2Model::RenderFrame( void )
{
	...
	while( int i = *(ptricmds++) )
	{
		...
		for( /* nothing */; i > 0; i--, ptricmds += 3 )
		{
			...
			glTexCoord2d( ((float *)ptricmds)[0], ((float *)ptricmds)[1] );
